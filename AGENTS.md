...
- **登录方式二选**：
  1. 管理界面「浏览器画面」内嵌操作真实页面登录（手机操作不便时可不用）；
  2. 直接粘贴电脑浏览器导出的「复制为 cURL」到 `/api/platform/import/{name}`：只提取该平台真实会用到的 Cookie 与请求头（Host/UA/Content-Length 等浏览器自带头会被剔除），注入浏览器 profile 后回放一次平台首页做登录态检测。签名类参数（a_bogus / PoW 等）仍由页面自身 JS 在请求时生成，无需逆向。
  - **导入路径不把任何 Cookie/请求头写入 config.toml 或日志**，仅注入运行时浏览器 profile（Cookie 随 profile 持久，导入的额外请求头只存在内存，重启后需重新导入）。
